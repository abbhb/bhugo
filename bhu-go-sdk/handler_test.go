...
func TestGetArgs(t *testing.T) {
	...
	handler.HandleFunc("/success", func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`<form id="loginForm" action="/tpass/login?service=https%3A%2F%2Fportal.beihua.edu.cn%2Ftp_up%2F" method="post">
...
func TestBuildAuthRequest(t *testing.T) {
	...
	r := buildAuthRequest("test", "test", "test",
		"https://pass.beihua.edu.cn/tpass/login?service=http%3A%2F%2F219.216.96.4%2Feams%2FhomeExt.action")
	res, _ := io.ReadAll(r.Body)
	_ = r.Body.Close()
	a.Equal("rsa=testtesttest&ul=4&pl=4&lt=test&execution=e1s1&_eventId=submit",
		string(res))
	a.Equal("https://pass.beihua.edu.cn/tpass/login?service=http%3A%2F%2F219.216.96.4%2Feams%2FhomeExt.action",
		r.URL.String())
}
